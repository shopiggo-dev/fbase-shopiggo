...
export async function getProductsFromCJ(query: string, advertiserId: string, linkId: string): Promise<Product[]> {
    ...
    const graphqlQuery = {
        query: `
            query ProductSearch($query: String, $advertiserIds: [ID!]) {
              products(
                keywords: $query,
    ...
    try {
        ...
        const products = jsonResponse.data?.products?.records || [];
...
function mapCjDataToProduct(item: any, publisherId: string, linkId: string): Product {
    ...
    const retailerInfo = retailerData.find(r => r.cid === item.advertiserId);
    const category = retailerInfo?.categories[0] || 'Apparel';
